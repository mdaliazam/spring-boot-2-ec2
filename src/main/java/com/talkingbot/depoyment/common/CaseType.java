...
public enum CaseType {
    ...
    public String changeCase(String val) {

        String returnValue = val;
        switch (this) {
            case LOWER:
                returnValue = val.toLowerCase(Locale.ROOT);
                break;

            case UPPER:
                returnValue = val.toUpperCase(Locale.ROOT);
                break;

            case TITLE:
                String[] words = val.split(whitespace_charclass);
                ...
                returnValue = String.join(" ", words);
                break;
            case SENTENCE:
                returnValue = Character.toUpperCase(val.charAt(0)) + val.substring(1);
                break;
